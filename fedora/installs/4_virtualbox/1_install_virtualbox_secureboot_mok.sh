...
# Find any .der key in the certs directory (handles public_key.der, fedora_*.der, akmods.der, etc.)
PUBLIC_DER=""
if [ -d "$CERT_DIR" ]; then
    PUBLIC_DER=$(find "$CERT_DIR" -name "*.der" -type f 2>/dev/null | head -n1)
fi

if [ -z "$PUBLIC_DER" ] || [ ! -f "$PUBLIC_DER" ]; then
    log_warn "No signing key found in $CERT_DIR"
    log_action "Generating akmods signing keys with kmodgenca..."
    kmodgenca -a
    PUBLIC_DER=$(find "$CERT_DIR" -name "*.der" -type f 2>/dev/null | head -n1)
    if [ -z "$PUBLIC_DER" ] || [ ! -f "$PUBLIC_DER" ]; then
        log_error "Failed to generate signing keys"
        exit 1
    fi
...
# Find the matching private key (handles .priv and .pem extensions, and mismatched basenames)
PRIVATE_KEY=""
for ext in priv pem; do
    for name in "$CERT_BASENAME" "private_key"; do
        if [ -f "$PRIVATE_DIR/${name}.${ext}" ]; then
            PRIVATE_KEY="$PRIVATE_DIR/${name}.${ext}"
            break 2
        fi
    done
done
if [ -z "$PRIVATE_KEY" ]; then
    PRIVATE_KEY=$(find "$PRIVATE_DIR" -type f \( -name "*.priv" -o -name "*.pem" \) 2>/dev/null | head -n1)
fi
if [ -z "$PRIVATE_KEY" ] || [ ! -f "$PRIVATE_KEY" ]; then
    log_error "No private key found in $PRIVATE_DIR"
    exit 1
fi
...
# Dynamically find akmods keys (public_key.der, fedora_*.der, akmods.der, etc.)
CERT_DIR="/etc/pki/akmods/certs"
PRIVATE_DIR="/etc/pki/akmods/private"

# Find any .der key file
PUBLIC_DER=$(find "$CERT_DIR" -name "*.der" -type f 2>/dev/null | head -n1)
...
# Find the matching private key (handles .priv and .pem, and mismatched basenames)
PRIVATE_KEY=""
for ext in priv pem; do
    for name in "$CERT_BASENAME" "private_key"; do
        if [ -f "$PRIVATE_DIR/${name}.${ext}" ]; then
...
if [ -z "$PRIVATE_KEY" ]; then
    echo "Error: No private key found in $PRIVATE_DIR"
    exit 1
fi
